...
class Store {
  ...
  // ----------------------------------------------------------------- consulta
  // match() y explain() comparten estos dos jueces a proposito: si divergieran,
  // el diagnostico mentiria justo cuando mas falta hace.
  // Devuelven null si pasa, o el nombre de la primera regla que lo tumba.
  serverReject(srv, filter, now) {
    if (filter.jobId && srv.jobId !== filter.jobId) return "jobId";
    if (filter.excludeJobIds && filter.excludeJobIds.has(srv.jobId)) return "excluido";
    if (filter.unclaimedOnly) {
      const c = this.claims.get(srv.jobId);
      if (c && c.expiresAt > now) return "ya reservado";
    }
    if (filter.hasSlot && srv.maxPlayers && srv.players >= srv.maxPlayers) return "server lleno";
    if (filter.maxPlayers != null && srv.players > filter.maxPlayers) return "demasiados jugadores";
    if (filter.freshServer != null && now - srv.updatedAt > filter.freshServer) return "server viejo";
    return null;
  }

  eggReject(egg, filter, now) {
    // Los dos cortes que usa el AJ para no saltar a hallazgos viejos.
    if (filter.sinceSeq != null && egg.seq <= filter.sinceSeq) return "anterior al cursor";
    if (filter.since != null && egg.firstSeen <= filter.since) return "anterior al cursor";
    if (filter.maxAgeMs != null && now - egg.firstSeen > filter.maxAgeMs) return "demasiado viejo";
    if (filter.minKg != null && egg.kg < filter.minKg) return "pesa poco";
    if (filter.maxKg != null && egg.kg > filter.maxKg) return "pesa mucho";
    if (filter.minRank != null && rarityRank(egg.rarity) < filter.minRank) return "rareza baja";
    if (filter.rarities && !filter.rarities.has(egg.rarity.toLowerCase())) return "rareza no marcada";
    if (filter.species && !filter.species.has((egg.species || egg.name).toLowerCase())) return "especie";
    return null;
  }
  ...
  // Por que no hay objetivo. Recorre el mismo embudo que match() y cuenta
  // cuantos huevos caen en cada regla, para que el AJ pueda decir
  // "hay 4 huevos pero 3 son de rareza no marcada y 1 es anterior al cursor"
  ...
  explain(filter) {
    ...
    // La razon que mas huevos tumba: es la que hay que enseñar primero.
    let top = null;
    ...
    return {
      ...
      top,
      cursor: this._eggSeq,
      // Un filtro de rareza que no cita ninguna rareza conocida no puede
      // acertar nunca: casi siempre es un fallo de configuracion, no falta
      // de huevos.
      rarityFilterUnknown:
