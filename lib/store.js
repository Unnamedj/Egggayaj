...
class Store {
  constructor(opts) {
    ...
    // Who is talking to the hub right now. A client is anything that names
    // itself: an AJ claiming targets, a reporter pushing eggs, a bot pulling
    // job ids from the pool. Kept so the dashboard can answer "is my auto
    // joiner actually connected?", which used to be guesswork.
    this.clients = new Map();
    this.clientTtlMs = opts.clientTtlMs || 10 * 60 * 1000;
  ...
  // ------------------------------------------------------------------ clients
  // Called on every identified request. `kind` is what the caller was doing,
  // not what it is: one process can both report eggs and claim targets, so the
  // roles accumulate instead of overwriting each other.
  touchClient(id, kind, extra) {
    const name = norm(id);
    if (!name) return null;
    const now = Date.now();
    ...
    if (!c) {
      ...
      this.log("client-up", { client: name, kind });
    }
    c.lastSeen = now;
    c.lastKind = kind || c.lastKind || "";
    if (kind) c.roles[kind] = (c.roles[kind] || 0) + 1;
    if (extra) {
      if (extra.ip) c.ip = extra.ip;
      if (extra.jobId) c.lastJobId = extra.jobId;
      for (const k of ["claims", "releases", "hops", "hopFails", "reports", "dispenses", "jobIds", "drops"]) {
        if (extra[k]) c[k] += extra[k];
      }
    }
    return c;
  }
  ...
  clientRows() {
    ...
    for (const c of this.clients.values()) {
      const idleMs = now - c.lastSeen;
      rows.push({
        id: c.id,
        roles: Object.keys(c.roles).sort((a, b) => c.roles[b] - c.roles[a]),
        lastKind: c.lastKind || "",
        ip: c.ip || "",
        ...
        hopFails: c.hopFails,
        reports: c.reports,
        dispenses: c.dispenses,
        jobIds: c.jobIds,
        drops: c.drops,
        firstSeen: c.firstSeen,
        lastSeen: c.lastSeen,
        idleMs,
        aliveMs: now - c.firstSeen,
        // "Online" is deliberately generous: an AJ long-polling /api/claim for
        // 55s at a time looks silent for most of a minute even when it is
        // perfectly healthy.
        online: idleMs < 90000,
      });
    }
  ...
  report(body, meta) {
    ...
    for (const raw of incoming) {
      ...
      if (egg.kg > this.stats.maxKg) this.stats.maxKg = egg.kg;
    ...
    if (fresh.length) {
      ...
      // Announcing happens on FRESH eggs only, which is what makes the Discord
      // relay safe to run with several reporters: the second one to find the
      // same egg produces nothing.
      if (this.onFreshEggs) {
        try { this.onFreshEggs(fresh, srv); } catch (_) {}
      }
    }
  ...
  prune() {
    ...
    for (const [id, c] of this.clients) {
      if (now - c.lastSeen > this.clientTtlMs) {
        this.clients.delete(id);
        this.log("client-down", { client: id, aliveMs: now - c.firstSeen });
      }
    }
  ...
  snapshot() {
    ...
    return {
      ...
      clientsOnline: [...this.clients.values()].filter((c) => now - c.lastSeen < 90000).length,
      ...
      // The heaviest egg ever reported. There is no way to pick a sensible
      // "insane kg" alert threshold without knowing the game's real scale, so
      // the hub reports it rather than leaving the number a guess.
      maxKg: this.stats.maxKg,
      stats: this.stats,
    };
  }
